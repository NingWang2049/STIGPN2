        ...
        for frame_index, frame_id in enumerate(coord_frame_list):
            ...
            for box_data in frame_data['labels']:
                ...
                try:
                    ...
                    pass
        object_set_count = 3 if len(object_set)-1 > 3 else len(object_set)-1
        num_objs = torch.tensor(object_set_count)
        return num_objs, box_tensors, box_categories


    def __getitem__(self, index):
        num_objs, box_tensors, box_categories = self.sample_single(index)
        return num_objs, box_tensors, box_categories, self.classes_dict[self.labels[index]]
