...
class VideoFolder(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        #self.prepare_data()
        if self.is_val:
            with open('/ssd/datasets/Something-Else/val_temp_data.pkl','rb') as f:
                temp = joblib.load(f)
        else:
            with open('//ssd/datasets/Something-Else/train_temp_data.pkl','rb') as f:
                temp = joblib.load(f)
        self.vid_names = temp['vid_names']
        self.labels = temp['labels']
        self.frame_cnts = temp['frame_cnts']
    ...
    def prepare_data(self):
        ...
        #temp = {}
        #temp['vid_names'] = vid_names
        #temp['labels'] = labels
        #temp['frame_cnts'] = frame_cnts
        #if self.is_val:
        #    with open('/ssd2/wn/something-else/val_temp_data.pkl','wb') as f:
        #        joblib.dump(temp,f)
        #else:
        #    with open('/ssd2/wn/something-else/train_temp_data.pkl','wb') as f:
        #        joblib.dump(temp,f)
    ...
    def sample_single(self, index):
        ...
        if self.is_val:
            video_path = '/ssd/datasets/Something-Else/val/'+folder_id+'.pkl'
        else:
            video_path = '/ssd/datasets/Something-Else/train/'+folder_id+'.pkl'
        with open(video_path,'rb') as f:
            data = joblib.load(f)
        video_node_features = data['video_node_features']
        img_wh = data['img_wh']
        video_node_features = torch.tensor(video_node_features).float()
        ...
        # frames.append(Image.new('RGB',img_wh))
        # height, width = frames[0].height, frames[0].width
        
        # frames = [img.resize((self.pre_resize_shape[1], self.pre_resize_shape[0]), Image.BILINEAR) for img in frames]

        height, width = img_wh[1], img_wh[0]
        frames.append(Image.new('RGB',self.pre_resize_shape))
        ...
        for frame_index, frame_id in enumerate(coord_frame_list):
            try:
                frame_data = video_data[frame_id]
                appearance_feats[:,frame_index,:] = video_node_features[:,frame_id,:]
            except:
                frame_data = {'labels': []}
            for box_data in frame_data['labels']:
                standard_category = box_data['standard_category']
                ...
                try:
                    ...
                    # load box category
                    box_categories[frame_index, global_box_id] = 1 if box_data['category'] == 'hand' else 2  # 0 is for none
                except:
                    pass

        return folder_id, frames,appearance_feats, box_tensors, box_categories


    def __getitem__(self, index):
        folder_id, frames,appearance_feats, box_tensors, box_categories = self.sample_single(index)
        ...
        return global_img_tensors,appearance_feats, box_tensors, box_categories, self.classes_dict[self.labels[index]], self.labels[index], folder_id
        # return global_img_tensors,appearance_feats, box_tensors, box_categories, self.classes_dict[self.labels[index]]
